...
export class ThermoHygrometerService {
  ...
  readData = async () => {
    ...
    const bus = await i2c.openPromisified(1); // Use bus number 1 for I2C-1
    await bus.writeByte(DHT20_ADDR, 0x00, 0x22); // Send read command to sensor
    await new Promise(resolve => setTimeout(resolve, 1000)); // Wait for sensor to provide data
    const data = await bus.readWord(DHT20_ADDR, 0); // Read data from sensor
    const temperature = ((data >> 2) / 10).toFixed(2);
    const humidity = ((data & 0x3) * 10).toFixed(2);
    await bus.close();
